...
(compiler-state
  (cl:defstruct (slot-descriptor (:type list))
    ...
    (type (err-uninit) :type (satisfies trivial-types:type-specifier-p))))
...
(defmacro declaim-accessor-types (class-name slot-descriptors)
  (flet ((slot-accessor-declamation (slot)
           (accessor-type-declamation class-name slot)))
